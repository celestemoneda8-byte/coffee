...
$sql = "SELECT 
    ...
    o.total_amount, 
    COALESCE(o.status, LOWER(o.order_status)) as status,
    COALESCE(o.payment_status, 'unpaid') as payment_status,
    o.created_at,
    ro.rider_id,
    ra.rider_name,
    ro.order_status as rider_status
FROM orders o
...
$ridersResult = $conn->query("SELECT rider_id, rider_name, phone FROM rider_accounts ORDER BY rider_name");
$riders = [];
if ($ridersResult) {
  ...
  <div class="d-flex min-vh-100">
    ...
    <main class="flex-fill p-4">
      <div class="container-fluid">
        ...
        <?php $pageTitle = 'Orders'; include 'header.php'; ?>
                           ...
                           <form action="order_actions.php" method="POST">
                               ...
                               <select name="status" class="form-select form-select-sm status-select 
                                   <?php 
                                      ...
                                      elseif($order['status']=='on-delivery') echo 'border-info text-info';
                                   ?>" 
                        ...
                        <td>
                            <?php if (!empty($order['rider_name'])): ?>
                                <div class="d-flex align-items-center gap-2">
                                    ...
                                    <div>
                                        <div class="fw-bold small"><?php echo htmlspecialchars($order['rider_name']); ?></div>
                                        <span class="badge bg-info small"><?php echo $order['rider_status'] ?? 'Assigned'; ?></span>
                                    </div>
                                </div>
                            <?php elseif (in_array($order['status'], ['ready', 'preparing'])): ?>
                                <form action="order_actions.php" method="POST" class="d-inline">
                                    <input type="hidden" name="action" value="assign_rider">
                                    <input type="hidden" name="order_id" value="<?php echo $order['id']; ?>">
                                    <select name="rider_id" class="form-select form-select-sm" onchange="this.form.submit()" style="width: 120px;">
