...
if (!$dbMode && isset($_GET['init']) && $_GET['init'] === '1') {
    $_SESSION['orders'] = [
        ['id'=>1, 'customer'=>'Mariel Deuda', 'email'=>'mariel@gmail.com', 'address'=>'123 Main St', 'total'=>120.00, 'status'=>'Completed', 'payment_status'=>'paid', 'created_at'=>date('Y-m-d H:i:s', strtotime('-3 days'))],
        ['id'=>2, 'customer'=>'Celeste Moneda', 'email'=>'celes@gmail.com', 'address'=>'456 Market Ave', 'total'=>110.00, 'status'=>'Completed', 'payment_status'=>'paid', 'created_at'=>date('Y-m-d H:i:s', strtotime('-2 days'))],
        ['id'=>3, 'customer'=>'Rejie Rosario', 'email'=>'rejie@gmail.com', 'address'=>'789 Side Rd', 'total'=>160.00, 'status'=>'Pending', 'payment_status'=>'unpaid', 'created_at'=>date('Y-m-d H:i:s', strtotime('-1 day'))],
    ];
...
if ($dbMode) {
    ...
    $counts['orders_pending'] = count(array_filter(
        $orders,
        fn($o) => in_array(strtolower($o['status'] ?? ''), ['pending', 'out for delivery'])
    ));
}
  ...
  <div class="container-fluid p-4">
    ...
    <?php if (!isset($_SESSION['loggedin']) || !$_SESSION['loggedin']): ?>
      <div class="alert alert-warning">
        Not logged in. 
        <?php if ($remoteIsLocal): ?>
          Local dev: <a href="?dev_login=1">dev login</a> | <a href="?init=1">init sample orders</a>
        <?php endif; ?>
      </div>
    <?php endif; ?>
