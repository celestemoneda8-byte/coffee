...
try {
    // Get order basic info (use status column as primary, fallback to order_status)
    $stmt = $conn->prepare("SELECT order_id, customer_id, total_amount, COALESCE(LOWER(status), LOWER(order_status)) AS status, order_status, created_at FROM orders WHERE order_id = ?");
    ...
    $stmt2 = $conn->prepare(
        "SELECT oi.order_item_id, oi.product_id, oi.quantity, oi.price,
                p.name AS name, COALESCE(p.image, '') AS image
         FROM order_items oi
