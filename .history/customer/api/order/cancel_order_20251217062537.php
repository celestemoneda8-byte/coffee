...
try {
    ...
    $stmt = $conn->prepare("SELECT order_status, status FROM orders WHERE order_id = ? AND customer_id = ?");
    ...
    $currentStatus = strtolower($row['status'] ?? $row['order_status'] ?? '');
    if ($currentStatus !== 'pending') {
        echo json_encode(['status' => 'error', 'message' => 'Only pending orders can be cancelled.']);
        exit;
    }

    // Update both status columns for sync
    $stmt2 = $conn->prepare("UPDATE orders SET order_status = 'Cancelled', status = 'cancelled' WHERE order_id = ?");
