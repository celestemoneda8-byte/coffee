...
try {
    ...
    $customer_name = 'Guest';
    $customer_email = '';
    $stmt_cust = $conn->prepare("SELECT fullname, email FROM customers WHERE customer_id = ?");
    if ($stmt_cust) {
        $stmt_cust->bind_param("i", $customer_id);
        $stmt_cust->execute();
        $cust_result = $stmt_cust->get_result();
        if ($cust_row = $cust_result->fetch_assoc()) {
            $customer_name = $cust_row['fullname'] ?? 'Guest';
            $customer_email = $cust_row['email'] ?? '';
        }
        $stmt_cust->close();
    }

    // Insert into orders (set both status columns for sync)
    $stmt = $conn->prepare("INSERT INTO orders (customer_id, customer_name, customer_email, total_amount, order_status, status) VALUES (?, ?, ?, ?, 'Pending', 'pending')");
    if (!$stmt) throw new Exception("Prepare failed (orders): " . $conn->error);
    $stmt->bind_param("issd", $customer_id, $customer_name, $customer_email, $computed_total);
