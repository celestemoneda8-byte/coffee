...
function loadMenu(category) {
    ...
    coffeeMenu
        ...
        .forEach((item, index) => {
            ...
            // Vertical card layout with centered image
            cell.innerHTML = `
            <div class="coffee-card">
                <div class="coffee-card-image">
                    <img src="${resolveImg(item.img)}" alt="${escapeHtml(item.name)}">
                </div>
                <div class="coffee-card-body">
                    <h5 class="coffee-name">${escapeHtml(item.name)}</h5>
                    <p class="coffee-price">₱${item.price.toFixed(2)}</p>
...
async function showOrderSummary(productId, qty) {
    try {
        ...
        if (typeof window.displayOrderSummaryModal === 'function') {
            window.displayOrderSummaryModal(cartItems, orderSummary);
        } else {
...
async function checkoutSelected() {
    ...
    if (typeof window.displayOrderSummaryModal !== 'function') {
        console.error('displayOrderSummaryModal not found');
        alert('Order modal not available. Please refresh the page.');
        return;
    }

    window.displayOrderSummaryModal(selectedItems);
}
...
async function loadCartPage() {
    ...
    window.SERVER_CART = SERVER_CART; // Keep global reference updated
...
// displayOrderSummaryModal will be defined by the IIFE below
window.removeFromCart = removeFromCart;
window.loadCartPage = loadCartPage;
window.updateCartBadge = updateCartBadge; // exported for other modules
window.SERVER_CART = SERVER_CART; // expose for checkoutSelected
