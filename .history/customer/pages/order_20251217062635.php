...
$pending = $conn->prepare("SELECT order_id, created_at, total_amount AS total, COALESCE(LOWER(status), LOWER(order_status)) AS status FROM orders WHERE customer_id = ? AND LOWER(COALESCE(status, order_status)) NOT IN ('completed','cancelled') ORDER BY created_at DESC");
$pending->bind_param("i", $userId);
$pending->execute();
$pendingResult = $pending->get_result();

// Fetch order history (completed or cancelled)
$history = $conn->prepare("SELECT order_id, created_at, total_amount AS total, COALESCE(LOWER(status), LOWER(order_status)) AS status FROM orders WHERE customer_id = ? AND LOWER(COALESCE(status, order_status)) IN ('completed','cancelled') ORDER BY created_at DESC");
