...
// Fetch pending orders with rider info
$pending = $conn->prepare("
    SELECT o.order_id, o.created_at, o.total_amount AS total, 
           COALESCE(LOWER(o.status), LOWER(o.order_status)) AS status,
           ro.rider_id, ra.rider_name, ra.phone as rider_phone, ro.order_status as rider_status
    FROM orders o
    LEFT JOIN rider_orders ro ON o.order_id = ro.order_id
    LEFT JOIN rider_accounts ra ON ro.rider_id = ra.rider_id
    WHERE o.customer_id = ? AND LOWER(COALESCE(o.status, o.order_status)) NOT IN ('completed','cancelled') 
...
$history = $conn->prepare("
    SELECT o.order_id, o.created_at, o.total_amount AS total, 
           COALESCE(LOWER(o.status), LOWER(o.order_status)) AS status,
           ro.rider_id, ra.rider_name
    FROM orders o
    LEFT JOIN rider_orders ro ON o.order_id = ro.order_id
    LEFT JOIN rider_accounts ra ON ro.rider_id = ra.rider_id
    WHERE o.customer_id = ? AND LOWER(COALESCE(o.status, o.order_status)) IN ('completed','cancelled') 
    ORDER BY o.created_at DESC
");
            ...
            <?php while($order = $pendingResult->fetch_assoc()): ?>
                <div class="col-md-6 mb-3">
                    <div class="card p-3">
                        ...
                        <div class="rider-info" style="background: #f8f9fa; padding: 10px; border-radius: 8px; margin-bottom: 10px;">
                            <p style="margin: 0; font-size: 14px;"><i class="bi bi-bicycle" style="color: #198754;"></i> <strong>Rider:</strong> <?= htmlspecialchars($order['rider_name']) ?></p>
                            <?php if (!empty($order['rider_phone'])): ?>
                            <p style="margin: 0; font-size: 13px; color: #666;"><i class="bi bi-telephone"></i> <?= htmlspecialchars($order['rider_phone']) ?></p>
                            <?php endif; ?>
                            <?php if (!empty($order['rider_status'])): ?>
                            <p style="margin: 0; font-size: 13px; color: #17a2b8;"><i class="bi bi-geo-alt"></i> Status: <?= htmlspecialchars($order['rider_status']) ?></p>
                            <?php endif; ?>
                        </div>
                        <?php endif; ?>

                        <!-- Progress Tracker with On-Delivery -->
                        <div class="order-tracker mt-2">
                            <?php
                            $statuses = ['pending','preparing','ready','on-delivery','completed'];
                            $currentStatus = strtolower($order['status']);
                            foreach ($statuses as $s):
                                $currentIndex = array_search($currentStatus, $statuses);
                                $stepIndex = array_search($s, $statuses);
                                $active = ($currentIndex !== false && $stepIndex <= $currentIndex) ? 'active' : '';
                                $label = $s === 'on-delivery' ? 'On Delivery' : ucfirst($s);
                            ?>
                                <div class="step <?= $active ?>"><?= $label ?></div>
                            <?php endforeach; ?>
                        </div>
                ...
                <div class="col-md-6 mb-3">
                    <div class="card p-3">
                        ...
                        <div class="order-tracker mt-2">
                            ...
                            $statuses = ['pending','preparing','ready','on-delivery','completed'];
                            $currentStatus = strtolower($order['status']);
                            // For cancelled orders, show cancelled badge instead
                            if ($currentStatus === 'cancelled'):
                            ?>
                                <div class="step" style="background:#dc3545;color:#fff;">Cancelled</div>
                            <?php else:
                                foreach ($statuses as $s):
                                    $currentIndex = array_search($currentStatus, $statuses);
                                    $stepIndex = array_search($s, $statuses);
                        ...
                        <?php if (!empty($order['rider_name'])): ?>
                        <p style="font-size: 13px; color: #666; margin-top: 8px;"><i class="bi bi-bicycle"></i> Delivered by: <?= htmlspecialchars($order['rider_name']) ?></p>
                        <?php endif; ?>
